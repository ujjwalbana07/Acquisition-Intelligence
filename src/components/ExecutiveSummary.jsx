...
export default function ExecutiveSummary({ inputs, metrics, result, risks, nextSteps }) {
  const [copied, setCopied] = useState(false);

  if (!result || !metrics || !inputs) return null;
  ...
  const risksText = risks?.length ? risks.map(r => `- [${r.severity.toUpperCase()}] ${r.title}: ${r.message}`).join('\n') : 'No material risks flagged.';
...
--------------------
Purchase Price:    ${formatCurrency(askingPrice)}
Required CapEx:    ${formatCurrency(renovationCost || 0)}
Total Capital:     ${formatCurrency(totalCapital)}

UNDERWRITING METRICS (STABILIZED)
---------------------------------
Entry Yield (Cap Rate):     ${formatPercent(capRate)}
Levered ROI (Cash-on-Cash): ${formatPercent(cashOnCash)}
Debt Service Coverage:      ${formatMultiple(dscr)}x
Leverage (LTV):             ${formatPercent(ltv)}
EXPENSE RATIO:              ${formatPercent(expenseRatio)}
            ...
            {risks?.map((risk, idx) => {
              ...
              return (
                <div key={idx} className={`p-4 rounded-lg bg-slate-900/40 border-l-[3px] border border-slate-800 ${style.border}`}>
                  ...
                  <p className="text-[11px] text-slate-400 leading-relaxed font-medium">{risk.message}</p>
                </div>
              );
            })}
          </div>
